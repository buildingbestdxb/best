...
export const careerForm = z
  .object({
    ...
    experience: z.preprocess(
      (val) => {
        // Convert empty string or whitespace to undefined so it triggers required error
        if (typeof val === "string" && val.trim() === "") return undefined;
        return typeof val === "string" ? Number(val) : val;
      },
      z
  ...
  .min(1, { message: 'This field is required' }),
    ...
    relativeName: z.string().min(1), // will be validated conditionally
    ...
    coverLetter: z.string().optional(), // conditionally required
    linkedinProfile: z
  .union([
